...
function addMenu(id, parent, label, icon, detail, dynamicSource) {
    var entry = {
        id: id,
        icon: icon || "•",
        label: label,
    ...
    menuInfo[id] = { id: id, label: label, type: "submenu", dynamicSource: dynamicSource || "" }
}
...
function addDynamicMenu(id, parent, label, icon, source) {
    addMenu(id, parent, label, icon, "", source)
}

function addLeaves(parent, labels, actionPrefix) {
    for (var i = 0; i < labels.length; i++) {
        var slug = labels[i].toLowerCase()
            .replace(/[^a-z0-9]+/g, "-")
            .replace(/^-+|-+$/g, "")
        var id = parent + "." + slug
...
addDynamicMenu("trigger.hardware", "trigger", "Hardware", "", "hardware")
...
for (var toggle of [
    ["Screensaver", "trigger-toggle-screensaver"], ["Nightlight", "trigger-toggle-nightlight"],
    ["Idle Lock", "trigger-toggle-idle"], ["Notifications", "trigger-toggle-notifications"],
    ["Top Bar", "trigger-toggle-bar"], ["Workspace Layout", "trigger-toggle-workspace-layout"],
    ["Window Gaps", "trigger-toggle-window-gaps"], ["1-Window Ratio", "trigger-toggle-window-ratio"],
    ["Monitor Scaling", "trigger-toggle-monitor-scaling"], ["Direct Boot", "trigger-toggle-direct-boot"]
]) addAction("trigger.toggle." + toggle[1], "trigger.toggle", toggle[0], "•", toggle[1])
...
// Style
addAction("style.theme", "style", "Theme", "•", "open-theme-switcher")
addDynamicMenu("style.unlocks", "style", "Unlock", "•", "unlocks")
addAction("style.background", "style", "Background", "•", "open-wallpaper-switcher")
addAction("style.hyprland", "style", "Hyprland", "•", "style-hyprland")
addDynamicMenu("style.font", "style", "Font", "•", "fonts")
...
for (var removeGroup of ["Development", "Browser", "Gaming"])
...
addAction("remove.theme", "remove", "Theme", "•", "remove-theme")
addAction("remove.dictation", "remove", "Dictation", "•", "remove-dictation")
addGroup("remove.security", "remove", "Security")
for (var removeSecurity of ["Fingerprint", "Fido2"])
    addAction("remove.security." + removeSecurity.toLowerCase(), "remove.security", removeSecurity, "•", "remove-security-" + removeSecurity.toLowerCase())
...
function dynamicSource(id) {
    return (menuInfo[id] || {}).dynamicSource || ""
}

...
| `~/.cache/qs-astra-notifications.json` | Compact `{ "recent": [...] }` history; `NotificationManager` queues snapshots through `qs-state-write`, which validates JSON and atomically replaces the private file |
...
| `${XDG_RUNTIME_DIR:-/tmp}/qs-astra-osd.json` | Ephemeral OSD bridge state |
| `${XDG_STATE_HOME:-~/.local/state}/qs-astra/mode` | Atomically replaced `quickshell` or `omarchy` profile |
| `${XDG_STATE_HOME:-~/.local/state}/qs-astra/notifications-silenced` | `qs-notification-silence`; one-line `0`/`1`, written by same-directory atomic rename with mode `0600` |
...
| `~/.config/quickshell/bar/.qsastra-source` | Optional source checkout path |

...
mkdir -p "$HOME/.config/quickshell/bar" "$(dirname "$HYPR_BINDINGS_CONF")" "$XDG_STATE_HOME/qs-astra" "$XDG_RUNTIME_DIR" "$tmp/bin"
...
mode_file="$XDG_STATE_HOME/qs-astra/mode"
...
if compgen -G "$XDG_STATE_HOME/qs-astra/.mode.*" >/dev/null; then
